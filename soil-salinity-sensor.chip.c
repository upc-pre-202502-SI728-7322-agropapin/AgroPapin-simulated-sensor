...
typedef struct {
  ...
  float salinity;      // salinity's parts per thousand attribute value (simulated)
} chip_data_t;
...
void chip_timer_callback(void *data) 
{
  ...
  float salinity = attr_read(chip_data->salinity);
...
void chip_init() 
{
  ...
  // Initialize the "salinity" control attribute with a default value
  chip_data->salinity = attr_init("salinity", 0.0);
